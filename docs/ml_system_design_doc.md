...
Ниже представлено заполнение **Этапа 1. Подготовка данных** для вашего ML System Design Doc, строго по запрошенному шаблону.

Оно адаптировано под вашу предметную область (фриланс-платформа, классификация текстов заявок и предсказание статуса сделки) и опирается на выводы проведенного EDA.

---

### Этап 1. Подготовка данных

В данном разделе описаны источники данных, необходимые для обучения модели скоринга заявок (Lead Scoring), а также статус их доступности и качества.
...
| :--- | :--- | :--- | :--- | :--- |
| **Целевая переменная**<br>(`deal_status` / `is_accepted`) | **Да**<br>`DWH.CRM_DEALS_HISTORY`<br>(Дамп таблицы заказов) | **Data Analyst / DS**<br>(SQL выгрузка) | **Частично (+/-)** | Таргет (Accepted/Rejected) присутствует. <br>**Проблема:** Выявлен дисбаланс классов (Accepted ~30%). Много записей в статусе `Pending` ("Цензурированные данные"), которые нужно либо исключить из обучения, либо использовать спец. методы (PU-learning). |
| **Текстовые признаки**<br>(`client_request`, `commercial_proposal`) | **Да**<br>`DWH.MESSAGES_LOG`<br>(Таблица переписки) | **Data Engineer**<br>(Настройка ETL для текстов) | **Да (+)** | Тексты полные, длина достаточная. <br>**Риск:** Возможны вкрапления HTML-тегов, смайликов и PII (персональных данных: телефоны, имена), требующие очистки на препроцессинге. |
...
| **Категориальные признаки**<br>(`sector`, `date`) | **Да**<br>`DWH.DEAL_METADATA` | **Data Analyst** | **Да (+)** | Сектора (Web, Design и др.) заполнены корректно без пропусков. Дата позволяет сгенерировать фичи сезонности (день недели, месяц). |
| **Внешние данные**<br>(Курсы валют / Инфляция) | **Нет** | **Data Engineer** | **Нет** | *Optional.* Возможно потребуется для нормализации бюджетов, если данные накоплены за несколько лет (сейчас не используется). |
...
Так как тексты заявок (`client_request`) могут содержать контакты и имена, перед передачей данных на обучение необходимо использовать **NER-маскирование** (замена сущностей на `<PERSON>`, `<PHONE>`), чтобы соответствовать GDPR.
...
Это отличное задание. Самая частая ошибка в ML System Design Doc — копипаста общих фраз ("почистим данные", "обучим xgboost").

Ниже приведено заполнение раздела **2.3 "Этапы решения задачи"**, полностью основанное на **специфике вашего датасета** (тексты заявок, грязное поле бюджет, дисбаланс классов) и выводах, полученных на этапе EDA.

Вы можете скопировать этот текст в свой документ.
...
На этом этапе мы переходим от сырых данных к артефактам ML-моделей.
...
*   **Техника решения:**
    ...
    *   **Модель:** Логистическая Регрессия (`LogisticRegression` из sklearn).
    *   **Валидация:** Hold-out (разбиение 80/20 со стратификацией по таргету, так как EDA показало дисбаланс 30/70).
*   **Ожидаемый результат:** Скрипт `train_baseline.py` и файл модели `logreg_v1.pkl`. Метрика ROC-AUC > 0.60. Выявление топ-20 слов, влияющих на принятие офера.
...
*Проверяем, не выучила ли модель ерунду.*
...
*Математическое качество (AUC) не равно деньгам.*
...
*Модель дает вероятность, но решение принимаем мы.*
